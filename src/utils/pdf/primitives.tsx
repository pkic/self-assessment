...
// `showShareLink` defaults to true so every existing self/extension caller
// (which never passes it) keeps rendering the "Go To Assessment" link
// unchanged; full-tier reports (whose assessment URL is lossy for a full
// assessment) pass `showShareLink={false}` to omit it.
export const Footer: React.FC<{
  assessmentUrl: string;
  version: string;
  showShareLink?: boolean;
}> = ({ assessmentUrl, version, showShareLink = true }) => (
...
// `showShareLink` defaults to true (same rationale as Footer above) so the
// QR block keeps rendering unchanged for every current caller; it also still
// requires `qrImgData` to be set, matching the pre-existing behavior.
export const CoverPage: React.FC<{
  version: string;
  reportTitle: string;
  qrImgData?: string | null;
  showShareLink?: boolean;
}> = ({ version, reportTitle, qrImgData, showShareLink = true }) => (
  <Page size="A4" style={styles.page}>
    ...
    <Text style={styles.subtitle_first}>{reportTitle}</Text>
...
export const DetailsTableHeader: React.FC = () => (
  <TableHeaderRow
    columns={[
