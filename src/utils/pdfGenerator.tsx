import {Document, G, Image, Link, Page, Path, pdf, StyleSheet, Svg, Text, View} from "@react-pdf/renderer";
...
interface PdfDocumentProps {
    ...
    modules: { id: string, name: string, categories: { id: string, name: string }[] }[];
...
const getColorForLevel = (level: number) => {
    switch (level) {
        case 1:
            return {
                background: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-1'),
                border: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-1'),
                text: '#ffffff'
            };
        case 2:
            return {
                background: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-2'),
                border: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-2'),
                text: '#ffffff'
            };
        case 3:
            return {
                background: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-3'),
                border: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-3'),
                text: '#000000'
            };
        case 4:
            return {
                background: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-4'),
                border: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-4'),
                text: '#000000'
            };
        case 5:
            return {
                background: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-5'),
                border: getComputedStyle(document.documentElement).getPropertyValue('--pkimm-maturity-level-5'),
                text: '#000000'
            };
        default:
            return {background: 'rgba(0, 0, 0, 0.1)', border: 'rgba(0, 0, 0, 1)', text: '#ffffff'};
    }
};

const headerColor = getComputedStyle(document.documentElement).getPropertyValue('--pkimm-primary-color');
...
const PdfDocument: React.FC<PdfDocumentProps> = ({
    ...
    <Document>
        <Page size="A4" style={styles.page}>
            ...
            <Text style={styles.title}>PKI Maturity Self-Assessment Report</Text>
            <View style={styles.overview_table}>
                <View style={[styles.overview_tableRow, {borderBottomWidth: 0, fontWeight: 'bold'}]}>
                    <View style={[styles.overview_tableCol, {width: '25%'}]}><Text style={styles.overview_tableCell}>PKI
                        Maturity Level:</Text></View>
                    <View style={[styles.overview_tableCol, {width: '75%'}]}><Text
                        style={[styles.overview_tableCell, {
                            color: getColorForLevel(overallMaturityLevel).background
                        }]}>{LevelResult[overallMaturityLevel]}</Text></View>
                </View>
                <View style={[styles.overview_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.overview_tableCol, {width: '25%'}]}><Text style={styles.overview_tableCell}>Assessment
                        Name:</Text></View>
                    <View style={[styles.overview_tableCol, {width: '75%'}]}><Text
                        style={styles.overview_tableCell}>{assessmentName}</Text></View>
                </View>
                <View style={[styles.overview_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.overview_tableCol, {width: '25%'}]}><Text style={styles.overview_tableCell}>Assessor
                        Name:</Text></View>
                    <View style={[styles.overview_tableCol, {width: '75%'}]}><Text
                        style={styles.overview_tableCell}>{assessorName}</Text></View>
                </View>
                <View style={styles.overview_tableRow}>
                    <View style={[styles.overview_tableCol, {width: '25%'}]}><Text style={[styles.overview_tableCell]}>Use
                        Case Description:</Text></View>
                    <View style={[styles.overview_tableCol, {width: '75%'}]}><Text
                        style={[styles.overview_tableCell]}>{useCaseDescription}</Text></View>
                </View>
            </View>
            <Image style={styles.chart} src={chartImgData}/>

            {/*break*/}
            <Text style={styles.heading} break>Overall PKI Maturity Level</Text>
            <View style={[styles.overallMaturityLevel, {
                ...
                }}>{LevelResult[overallMaturityLevel]}</Text>
            </View>
            ...
            <View style={styles.rectangleRow}>
                {moduleMaturityLevels.map(({module, level}) => (
                    <View style={[styles.rectangle, {
                        ...
                        <Text
                            style={{color: getColorForLevel(level).text, fontSize: 15, marginBottom: 5}}>{module}</Text>
                        <Text style={{color: getColorForLevel(level).text, fontSize: 15}}>{LevelResult[level]}</Text>
                    </View>
                ))}
            </View>
            <Text style={styles.heading}>Details</Text>
            <View style={styles.table}>
                <View style={[styles.tableRow, {borderTopWidth: 1, backgroundColor: headerColor, color: '#ffffff'}]}>
                    ...
                    module.categories.map((category, categoryIndex) => {
                        ...
                        return (
                            <View>
                                ...
                                >
                                    <View style={[styles.tableCol, {width: '5%'}]}>
                                        <Text style={styles.tableCell}>{`${module.id}.${category.id}`}</Text>
                                    </View>
                                    <View style={[styles.tableCol, {width: '11%'}]}>
                                    ...
                                    <View style={[styles.tableCol, {width: '12%'}]}>
                                        <Text
                                            style={[styles.tableCell, {color: getColorForLevel(progress[`${module.id}.${category.id}`]?.level).background}]}>
                                    ...
                                    <View style={[styles.tableCol, {width: '47%'}]}>
                                        <Text
                                            ...
                                            {progress[`${module.id}.${category.id}`]?.description || 'N/A'}
                                        </Text>
                                    </View>
                                </View>
                            </View>
            ...
            {/*break*/}
            <Text style={styles.about_heading} break>About PKI Maturity Model</Text>
            <Text style={styles.about_text}>
                The maturity model is based on the Capability Maturity Model Integration (CMMI) developed by Carnegie
                Mellon University. It should provide the following:
            </Text>
            <Text style={styles.about_text}>
                <ListItem>Quickly understand the current level of capabilities and performance of the PKI</ListItem>
                <Br/>
                <ListItem>Support comparison of PKI maturity with similar organizations based on size or industry
                    (anonymized)</ListItem>
                <Br/>
                <ListItem>Guidance on how to improve the capabilities of the current PKI</ListItem>
                <Br/>
            ...
            <Text style={styles.about_text}>
                <Br/>
                The PKI maturity model defines 5 levels of the PKI maturity based on different indicators and associated
                risks.
                <Br/><Br/>
            </Text>

            <View style={styles.maturity_table}>
                <View style={[styles.maturity_tableRow, styles.greyBackground, {
                    borderBottomWidth: 0,
                    fontWeight: 'bold'
                }]}>
                    <View style={[styles.maturity_tableCol, {width: '15%'}]}><Text style={styles.maturity_tableCell}>Maturity
                        level</Text></View>
                    <View style={[styles.maturity_tableCol, {width: '85%'}]}><Text style={styles.maturity_tableCell}>Short
                        description</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '15%'}]}><Text
                        style={styles.maturity_tableCell}>Initial</Text></View>
                    <View style={[styles.maturity_tableCol, {width: '85%'}]}><Text style={styles.maturity_tableCell}>Unpredictable
                        process with poor control and always reactive</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '15%'}]}><Text
                        style={styles.maturity_tableCell}>Basic</Text></View>
                    <View style={[styles.maturity_tableCol, {width: '85%'}]}><Text style={styles.maturity_tableCell}>Process
                        is characterized by each particular case or project and controls are often
                        reactive</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '15%'}]}><Text
                        style={styles.maturity_tableCell}>Advanced</Text></View>
                    <View style={[styles.maturity_tableCol, {width: '85%'}]}><Text style={styles.maturity_tableCell}>Process
                        is characterized by organizational standards and controls are proactive</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '15%'}]}><Text
                        style={styles.maturity_tableCell}>Managed</Text></View>
                    <View style={[styles.maturity_tableCol, {width: '85%'}]}><Text style={styles.maturity_tableCell}>Processes
                        are measured and controlled, proactive approach</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 1}]}>
                    <View style={[styles.maturity_tableCol, {width: '15%'}]}><Text
                        style={styles.maturity_tableCell}>Optimized</Text></View>
                    <View style={[styles.maturity_tableCol, {width: '85%'}]}><Text style={styles.maturity_tableCell}>Continuous
                        improvement of the processes and procedures, proactive approach for future technology
            ...
            <Text style={styles.about_text}>
                The PKI Consortium is comprised of leading organizations that are committed to improve,
                create, and collaborate on generic, industry or use-case specific policies, procedures, best practices,
                standards, and tools that advance trust in assets and communication for everyone and everything using
                Public Key Infrastructure (PKI) as well as the security of the internet in general. By engaging with
                users, regulators, supervisory bodies and other interested or relying parties the consortium
            ...
            <View style={styles.maturity_table}>
                <View style={[styles.maturity_tableRow, styles.greyBackground, {
                    ...
                    <View style={[styles.maturity_tableCol, {width: '25%'}]}><Text
                        style={styles.maturity_tableCell}>Resource</Text></View>
                    <View style={[styles.maturity_tableCol, {width: '75%'}]}><Text
                        style={styles.maturity_tableCell}>Description</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '25%'}]}><Text
                        style={styles.maturity_tableCell}><Link src="https://pkic.org">PKI
                        Consortium</Link></Text></View>
                    <View style={[styles.maturity_tableCol, {width: '75%'}]}><Text style={styles.maturity_tableCell}>PKI
                        Consortium home page</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '25%'}]}><Text
                        style={styles.maturity_tableCell}><Link src="https://pkic.org/pkimm/model/">PKI maturity
                        model</Link></Text></View>
                    <View style={[styles.maturity_tableCol, {width: '75%'}]}><Text style={styles.maturity_tableCell}>Definition
                        of the PKI maturity model and description of the maturity assessment process and procedures in
                        order to rate the current maturity level and to track progress</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '25%'}]}><Text
                        style={styles.maturity_tableCell}><Link src="https://pkic.org/pkimm/categories/">Categories
                        description</Link></Text></View>
                    <View style={[styles.maturity_tableCol, {width: '75%'}]}><Text style={styles.maturity_tableCell}>Description
                        of PKI maturity model related categories and associated requirement, guidance, assessment tips,
                        and references</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '25%'}]}><Text
                        style={styles.maturity_tableCell}><Link src="https://pkic.org/pkimm/assessment/">PKI maturity
                        assessment process</Link></Text></View>
                    <View style={[styles.maturity_tableCol, {width: '75%'}]}><Text style={styles.maturity_tableCell}>Description
                        of the assessment process</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '25%'}]}><Text
                        style={styles.maturity_tableCell}><Link src="https://pkic.org/pkimm/tools/">PKI maturity
                        assessment tools</Link></Text></View>
                    <View style={[styles.maturity_tableCol, {width: '75%'}]}><Text style={styles.maturity_tableCell}>Available
                        tools for the assessment of the PKI implementation and use case</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 0}]}>
                    <View style={[styles.maturity_tableCol, {width: '25%'}]}><Text
                        style={styles.maturity_tableCell}><Link src="https://forms.gle/7CgvuNoxaiTYbtK29">Feedback
                        form</Link></Text></View>
                    <View style={[styles.maturity_tableCol, {width: '75%'}]}><Text style={styles.maturity_tableCell}>PKI
                        maturity model and assessment feedback form</Text></View>
                </View>
                <View style={[styles.maturity_tableRow, {borderBottomWidth: 1}]}>
                    <View style={[styles.maturity_tableCol, {width: '25%'}]}><Text
                        style={styles.maturity_tableCell}><Link
                        src="https://github.com/orgs/pkic/discussions/categories/pki-maturity-model-pkimm">PKI maturity
                        model community discussion</Link></Text></View>
                    <View style={[styles.maturity_tableCol, {width: '75%'}]}><Text style={styles.maturity_tableCell}>Ideas,
                        questions, or feedback that you want to share or discuss related to the PKI maturity
                        model.</Text></View>
                </View>
            </View>
            ...
            <View>
                ...
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    <Link src="https://twitter.com/PKIConsortium"><FontAwesomeIcon faIcon={faXTwitter} style={{
                        color: '#000000',
                        width: '20px',
                        marginRight: '10px'
                    }}/></Link>
                    <Link src="https://www.linkedin.com/groups/4852478/"><FontAwesomeIcon faIcon={faLinkedin} style={{
                        color: '#0077B5',
                        width: '20px',
                        marginRight: '10px'
                    }}/></Link>
                    <Link src="https://github.com/pkic"><FontAwesomeIcon faIcon={faGithub} style={{
                        color: '#6e5494',
                        width: '20px'
                    }}/></Link>
                </View>
