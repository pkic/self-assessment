...
import {calculateOverallMaturityLevel, calculateModuleMaturityLevels} from '../../utils/maturityCalculations'; // Import utilities
...
export const Assessment: React.FC<AssessmentProps> = ({src}) => {
    ...
    function initProgress(parsedData: AssessmentData | null): Record<string, ProgressData> {
        return parsedData?.modules.reduce((acc, module) => {
    ...
    const handleLevelChange = (moduleId: string, categoryId: string, level: number) => {
        setProgress(prevProgress => ({
            ...
            [`${moduleId}.${categoryId}`]: {
                ...
                description: data?.modules.find(module => module.id === moduleId)?.categories.find(category => category.id === categoryId)?.levels.find(l => l.number === level)?.description || defaultProgressData.description,
                applicability: prevProgress[`${moduleId}.${categoryId}`]?.applicability || defaultProgressData.applicability,
            }
        }));
    ...
    const handleApplicabilityChange = (moduleId: string, categoryId: string) => {
        setProgress(prevProgress => ({
            ...prevProgress,
            [`${moduleId}.${categoryId}`]: {
                level: prevProgress[`${moduleId}.${categoryId}`]?.level || defaultProgressData.level,
                description: prevProgress[`${moduleId}.${categoryId}`]?.description || defaultProgressData.description,
                result: prevProgress[`${moduleId}.${categoryId}`]?.applicability ? LevelResult[-1] : LevelResult[prevProgress[`${moduleId}.${categoryId}`]?.level],
                applicability: !prevProgress[`${moduleId}.${categoryId}`]?.applicability,
            }
        }));
    };
    ...
    const handleExportPDF = () => {
        if (chartRef.current && data) {
            const overallMaturityLevel = calculateOverallMaturityLevel(data.modules, progress);
            const moduleMaturityLevels = calculateModuleMaturityLevels(data.modules, progress);
            exportToPDF(progress, chartRef.current, overallMaturityLevel, moduleMaturityLevels, data.modules,
                assessmentName, assessorName, useCaseDescription);
        }
    };
    ...
    const handleTabClick = (tab: string) => {
        ...
        const scrollQuerySelector = getComputedStyle(document.documentElement).getPropertyValue('--pkimm-scroll-query-selector');
    ...
    const chartLabels = data?.modules.reduce((acc, module) => {
    ...
    const getNextModuleId = () => {
        ...
        const currentIndex = data.modules.findIndex(module => module.id === currentTab);
        if (currentIndex === -1 || currentIndex === data.modules.length - 1) return null;
        return data.modules[currentIndex + 1].id;
    };

    const nextModuleId = getNextModuleId();

    const overallMaturityLevel = data ? calculateOverallMaturityLevel(data.modules, progress) : 0;
    const moduleMaturityLevels = data ? calculateModuleMaturityLevels(data.modules, progress) : [];

    return (
        <div className="pkimm-assessment-container">
            <div className="pkimm-burger-menu" onClick={() => setIsMenuOpen(!isMenuOpen)}>
            ...
            <div className={`pkimm-tabs ${isMenuOpen ? 'active' : ''}`}>
                ...
                <img
                    ...
                    alt="PKI Consortium Logo" className="pkimm-logo"/>
            </div>
