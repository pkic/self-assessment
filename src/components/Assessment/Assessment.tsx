...
export const Assessment: React.FC<AssessmentProps> = ({
  ...
  const [target, setTarget] = useState<{ kind: 'original' } | { kind: 'extension', id: string }>({ kind: 'original' });
    ...
    const loadData = async () => {
      ...
      if (encodedProgress) {
        try {
          ...
          let initialExtensions: ExtensionData[] = [];
          if (extensions) {
          ...
          if (initialData) {
            const baseProgress = initProgress(initialData, initialExtensions, decodedData.enabledExtensions || []);
        ...
        if (storedData) {
          ...
          if (initialData) {
            const baseProgress = initProgress(initialData, initialExtensions, storedEnabledExtensions || []);
          ...
          <div className="pkimm-categories-container">
            ...
            {currentTab === "report" && data && (
              <UnifiedReport
                ...
                enabledExtensions={enabledExtensions}
                onExportPDF={(extId) => extId ? handleExportExtensionPDF(extId) : handleExportPDF()}
                onReset={(extId) => extId ? handleResetExtension(extId) : handleReset()}
